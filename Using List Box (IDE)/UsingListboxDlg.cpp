...
void UsingListboxDlg::Cls_OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
	...
	case IDC_EDIT:
	{
		...
		if (index != LB_ERR)
		{
			...
			// Заполним поле редактирования текстом
			SetWindowText(hEditItem, pBuffer);

			// Покажем поле редактирования
			ShowWindow(hEditItem, SW_SHOW);

			// Сделаем поле редактирования активным
			SetFocus(hEditItem);
	...
	case IDC_EDIT_ITEM:
	{
		...
		{
			// Спрячем поле редактирования
			ShowWindow(hEditItem, SW_HIDE);

			// Получим текст из поля редактирования
			int length = GetWindowTextLength(hEditItem);
			TCHAR* pBuffer = new TCHAR[length + 1];
			GetWindowText(hEditItem, pBuffer, length + 1);

			// Обновим текст элемента списка
			SendMessage(hList1, LB_DELETESTRING, index, 0);
...
BOOL UsingListboxDlg::Cls_OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
	...
	// Создадим кнопку "Редактировать"
	hEdit = CreateWindowEx(0, WC_BUTTON, TEXT("Редактировать"), WS_CHILD | WS_VISIBLE | BS_FLAT,
		104, 60, 130, 17, hwnd, (HMENU)IDC_EDIT, GetModuleHandle(NULL), NULL);

	// Создадим поле редактирования
	hEditItem = CreateWindowEx(0, WC_EDIT, TEXT(""), WS_CHILD | WS_BORDER | ES_AUTOHSCROLL,
		104, 81, 130, 14, hwnd, (HMENU)IDC_EDIT_ITEM, GetModuleHandle(NULL), NULL);
